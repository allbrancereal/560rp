...
SWEP.Purpose = "Save a life, rescue someone."
				...
				if !v:Alive() then
					for _, ent in pairs(ents.FindInSphere(EyeTrace.HitPos, 5)) do						
						if ent == v:GetRagdollEntity() then
							...
							self.Weapon:EmitSound("fsrp/actions/revive.wav");
...
util.AddNetworkString('defibCall');

net.Receive("defibCall", function( len , _p )

	_p:DefibPlayer( net.ReadString() );

			
	local _skChance = math.random( 10000 )
	if _skChance < 1001 then
...
function SWEP:SecondaryAttack()
	...
	if (LocalPlayer().__LastChargeUp && LocalPlayer().__LastChargeUp + .75 > CurTime()) then return; end
	...
	LocalPlayer().__LastChargeUp = CurTime();
