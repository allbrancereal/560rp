...
CustomizableWeaponry.callbacks:addNew("holsterWeapon", "messageonDeploy", function( self )
	
	if CLIENT then
	
	local _p = LocalPlayer()
			if _p.WeaponWheel  then _p.WeaponWheel:Update() end
	if _p:GetActiveWeapon() != self then return end
	local _pronoun = "her";
	if _p:getGender() != 1 then
		...
		for k , v in pairs( ITEMLIST ) do
			...
			if v.WeaponClass == self:GetClass() then
		
				_p:ConCommand( "say /me has holstered " .. _pronoun .. " " .. v.Name );
...
CustomizableWeaponry.callbacks:addNew("deployWeapon", "messageonDeploy", function( self )
	...
	if _p:getGender() != 1 then
		...
		for k , v in pairs( ITEMLIST ) do
			...
			if v.WeaponClass == self:GetClass() then
				...
				_p:ConCommand( "say /me has deployed " .. _pronoun .. " " .. v.Name );
							...
							for k, v in pairs(player.GetAll()) do
								if (!v:Alive()) then
									...
									if (playerRagdoll) then
										for _, ent in pairs(ents.FindInSphere(trace.HitPos, 5)) do						
											if ent == playerRagdoll then
												net.Start( "tryExecution")
